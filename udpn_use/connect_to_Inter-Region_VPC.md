...
假如我们需要华南（广州）的VPC1联通在华北（北京2）的VPC2，购买华南（广州）-华北（北京2）（或者华北（北京2）-华南（广州）都是一样的）的线路即可，UDPN操作见[使用指南](/udpn/guide) 。
![](/images/create_udpn.png)

如果已经购买了需要打通地域的UDPN线路，则直接进行下一步。

### 2、进行VPC打通
UDPN网络互通页面，华南（广州）选中VPC1，点击**VPC联通管理**，在弹出的页面中选中需要联通的VPC2，点击确认，即可完成跨地域内网联通。
